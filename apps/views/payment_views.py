...
class SuperAdminFinanceSummaryView(APIView):
    ...
    def get(self, request):
        ...
        last_month_debt = Debt.objects.filter(
            status__in=[Debt.Status.UNPAID, Debt.Status.PARTIALLY_PAID],
            due_date__gte=last_month_start.date(),
            due_date__lte=last_month_end.date(),
        ).aggregate(t=Sum("amount"))["t"] or 0

        pending_debts_change = round(
            (pending_debts - last_month_debt) / last_month_debt * 100, 1
        ) if last_month_debt else 0.0
...
class SuperAdminFinanceCentersView(APIView):
    ...
    def get(self, request):
        ...
        total = qs.count()
        total_revenue_sum = Payment.objects.filter(
            status=Payment.Status.PAID
        ).aggregate(t=Sum("final_amount"))["t"] or 0
