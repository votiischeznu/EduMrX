...
async def command_start_handler(message: types.Message, command: CommandObject):
    ...
    if not link_token:
        await message.answer(
            "Xush kelibsiz! Tizimga kirish yoki parolni tiklash uchun saytdan foydalaning."
        )
        return
    ...
    if link_token.startswith("rec_"):
        ...
        await redis_client.setex(
            f"otp_verification:{user_id}", 300, json.dumps(payload)
        )
        ...
        await redis_client.setex(
            f"otp_verification:{phone}", 300, json.dumps(existing_data)
        )
